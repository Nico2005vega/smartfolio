...
const FONTS = [
  // Sans-serif
  { name:"Inter",       family:"Inter,system-ui,sans-serif",              cat:"Sans",   preview:"Inter — Aa"        },
  { name:"Poppins",     family:"Poppins,system-ui,sans-serif",            cat:"Sans",   preview:"Poppins — Aa"      },
  { name:"Montserrat",  family:"Montserrat,system-ui,sans-serif",         cat:"Sans",   preview:"Montserrat — Aa"   },
  { name:"Raleway",     family:"Raleway,system-ui,sans-serif",            cat:"Sans",   preview:"Raleway — Aa"      },
  { name:"Nunito",      family:"Nunito,system-ui,sans-serif",             cat:"Sans",   preview:"Nunito — Aa"       },
  { name:"Lato",        family:"Lato,system-ui,sans-serif",               cat:"Sans",   preview:"Lato — Aa"         },
  { name:"Roboto",      family:"Roboto,system-ui,sans-serif",             cat:"Sans",   preview:"Roboto — Aa"       },
  { name:"Open Sans",   family:"'Open Sans',system-ui,sans-serif",        cat:"Sans",   preview:"Open Sans — Aa"    },
  // Serif
  { name:"Playfair",    family:"'Playfair Display',Georgia,serif",        cat:"Serif",  preview:"Playfair — Aa"     },
  { name:"Merriweather",family:"Merriweather,Georgia,serif",              cat:"Serif",  preview:"Merriweather — Aa" },
  { name:"Lora",        family:"Lora,Georgia,serif",                      cat:"Serif",  preview:"Lora — Aa"         },
  { name:"EB Garamond", family:"'EB Garamond',Georgia,serif",             cat:"Serif",  preview:"Garamond — Aa"     },
  { name:"Georgia",     family:"Georgia,serif",                           cat:"Serif",  preview:"Georgia — Aa"      },
  // Mono
  { name:"JetBrains",   family:"'JetBrains Mono','Courier New',monospace",cat:"Mono",   preview:"JetBrains — Aa"   },
  { name:"Fira Code",   family:"'Fira Code','Courier New',monospace",     cat:"Mono",   preview:"Fira Code — Aa"   },
  { name:"Courier",     family:"'Courier New',monospace",                 cat:"Mono",   preview:"Courier — Aa"      },
];
...
const CATALOGUE: TplMeta[] = [
  ...
  { key:"academic",  name:"Académica",   category:"académico",    description:"Estilo Harvard/Oxford para investigadores",tags:["Académico","Denso"],      ats:94, accent:"#4f46e5" },
];
...
const TAG_COLORS: Record<string,string> = {
  ...
  Académico:"bg-violet-100 text-violet-700", Denso:"bg-gray-100 text-gray-600",
};
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden" style={{ animation:"slideUp .22s cubic-bezier(.34,1.56,.64,1)" }}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{CATALOGUE.length} diseños exclusivos · Clic para aplicar al instante</p>
          </div>
              ...
              return (
                <div key={tpl.key}
                  ...
                  <div className="relative bg-gray-50" style={{ aspectRatio:"4/5" }}>
                    <Thumb k={tpl.key} c={tpl.accent ?? "#059669"}/>

                    {/* Hover overlay */}
                    {isHov && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2" style={{ background:"rgba(0,0,0,.45)", backdropFilter:"blur(1px)" }}>
                        <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white rounded-full shadow-lg"
                          style={{ background:tpl.accent ?? "#059669" }}
...
export default function CVBuilderClient({ profile, records, skills, templates, config, preSelectedTemplate }: Props) {
  ...
  const extConfig = useMemo(() => ({
    ...
    font_name:fontFamily, font_family:fontFamily.includes("Georgia")||fontFamily.includes("Playfair")||fontFamily.includes("Merriweather")||fontFamily.includes("Lora")||fontFamily.includes("Garamond")?"serif":fontFamily.includes("Mono")||fontFamily.includes("Code")||fontFamily.includes("Courier")?"mono":"sans",
  ...
  return (
    <>
      ...
      {galleryOpen && (
        <GalleryModal currentKey={tplKey} onSelect={key => { setTplKey(key); toast.success(`Plantilla "${CATALOGUE.find(t=>t.key===key)?.name}" aplicada ✓`); }} onClose={() => setGalleryOpen(false)}/>
      )}
                    ...
                    <div className="space-y-1.5">
                      {filteredFonts.map(f => (
                        <button key={f.name} onClick={() => { setFontName(f.name); setFontFamily(f.family); }}
                          className={cn("w-full flex items-center justify-between px-3 py-2.5 rounded-2xl border-2 transition-all",
                            fontName===f.name?"border-green-500 bg-green-50":"border-gray-100 hover:border-gray-200")}>
                          <div className="flex items-center gap-3">
                            <span className="text-xl font-bold text-gray-700 leading-none w-10 flex-shrink-0" style={{ fontFamily:f.family }}>
                              Aa
                            </span>
                          ...
                          {fontName===f.name && <CheckCircle2 size={14} className="text-green-500 flex-shrink-0"/>}
                        </button>
                      ))}
                    </div>
                ...
                {tab==="estilo" && (
                  <>
                    ...
                    <div className="flex items-center gap-2.5 p-2.5 bg-gray-50 rounded-2xl">
                      <input type="color" value={accent} onChange={e => setAccent(e.target.value)} className="w-9 h-9 rounded-xl cursor-pointer border-0 bg-transparent flex-shrink-0" style={{ padding:0 }}/>
                    ...
                    <div className="grid grid-cols-5 gap-1.5">
                      {(["solid","dashed","dotted","double","none"] as const).map(ds => (
                        <button key={ds} onClick={() => setDivider(ds)}
                          ...
                          <div style={{ width:"100%", height:0, borderTop:`2px ${ds==="none"?"solid":"${ds}"} ${dividerStyle===ds?"#16a34a":"#d1d5db"}`,
                            opacity:ds==="none"?0:1, borderTopStyle:ds as any }}/>
          ...
          <div className="flex flex-col min-h-0">
            <div className="flex items-center justify-between mb-3 px-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400">{fontName} {fontSize}px</span>
              </div>
            </div>
