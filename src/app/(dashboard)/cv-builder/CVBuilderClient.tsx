...
import {
  ...
  User, Wrench, Award, ChevronRight, BookOpen,
  Circle, Square, RectangleHorizontal,
  Minus, AlignLeft, Tag, BarChart2,
} from "lucide-react";
  ...
  switch (k) {
    case "modern": return (
      <svg viewBox="0 0 80 96" className="w-full h-full">
        <rect width="80" height="96" fill="#f1f5f9" rx="4"/>
        <rect x="0" y="0" width="24" height="96" fill={c}/>
        <circle cx="12" cy="18" r="7" fill="white" fillOpacity=".3"/>
        <rect x="3" y="30" width="16" height="1.5" fill="white" fillOpacity=".5" rx="1"/>
        <rect x="3" y="35" width="12" height="1.5" fill="white" fillOpacity=".4" rx="1"/>
        {[45,53,61,69,77,85].map((y,i)=>(
          <rect key={y} x="30" y={y-8} width={36-(i%3)*5} height="2.2" fill="#cbd5e1" rx="1"/>
        ))}
      </svg>
    );
    case "classic": return (
      <svg viewBox="0 0 80 96" className="w-full h-full">
        <rect width="80" height="96" fill="#f8fafc" rx="4"/>
        <rect x="14" y="9" width="52" height="6" fill={c} rx="2"/>
        <rect x="8" y="19" width="64" height=".8" fill={c} fillOpacity=".4"/>
        {[27,35,43,51,59,67,75].map((y,i)=>(
          <rect key={y} x="8" y={y} width={52-(i%4)*7} height="2" fill="#e2e8f0" rx="1"/>
        ))}
      </svg>
    );
    case "executive": return (
      <svg viewBox="0 0 80 96" className="w-full h-full">
        <rect width="80" height="96" fill="#fafafa" rx="4"/>
        <rect x="8" y="9" width="42" height="7" fill="#111827" rx="1"/>
        <rect x="8" y="20" width="24" height="2" fill={c} rx="1"/>
        {[32,42,52,62,72,82].map((y,i)=>(
          <rect key={y} x="8" y={y-4} width={55-(i%3)*8} height="2" fill="#e2e8f0" rx="1"/>
        ))}
      </svg>
    );
    case "creative": return (
      <svg viewBox="0 0 80 96" className="w-full h-full">
        ...
        <rect x="27" y="14" width="18" height="2" fill="white" fillOpacity=".65" rx="1"/>
        <rect x="8" y="33" width="3" height="52" fill="#e2e8f0" rx="1"/>
    ...
    case "tech": return (
      <svg viewBox="0 0 80 96" className="w-full h-full">
        <rect width="80" height="96" fill="#f8fafc" rx="4"/>
        <rect x="0" y="0" width="22" height="96" fill="#0f172a"/>
        <circle cx="11" cy="16" r="5.5" fill={c} fillOpacity=".35"/>
        {[28,35,42,50].map(y=>(
          <rect key={y} x="4" y={y} width="13" height="1.5" fill={c} fillOpacity=".45" rx="1"/>
        ))}
        <rect x="26" y="8" width="44" height="1.5" fill={c} rx="1"/>
        {[16,25,34,43,52,61,70,79].map((y,i)=>(
          <rect key={y} x="28" y={y} width={36-(i%3)*5} height="2" fill="#e2e8f0" rx="1"/>
        ))}
      </svg>
    );
    default: return <div className="w-full h-full bg-gray-100 rounded"/>;
  }
}
            ...
            <div className="p-4 space-y-4 max-h-[72vh] overflow-y-auto">
                ...
                <>
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {PALETTES.map(hex => (
                      <button key={hex} onClick={() => setAccent(hex)} title={hex}
                        className={cn(
                          "aspect-square rounded-xl transition-all hover:scale-110",
                  ...
                  <div className="flex items-center gap-2.5 p-2.5 bg-gray-50 rounded-2xl">
                    ...
                    <div className="w-10 h-9 rounded-xl overflow-hidden border border-gray-200 flex-shrink-0">
                      ...
                      <div className="h-1/2 bg-white flex items-center px-1.5">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {[
                      ...
                      <button key={key} onClick={() => setFont(key as any)}
                        className={cn(
                          "p-2.5 rounded-2xl border-2 text-left transition-all",
              ...
              {tab === "diseno" && (
                <>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <button key={key} onClick={() => setPhotoShape(key as any)}
                          className={cn(
                            "p-2.5 rounded-2xl border-2 flex flex-col items-center gap-1.5 transition-all hover:shadow-sm",
                            photoShape === key ? "border-green-500 bg-green-50" : "border-gray-100 hover:border-gray-200"
                          )}>
                          <div className="w-9 h-9 bg-gradient-to-br from-gray-300 to-gray-200 flex items-center justify-center"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { key:"underline", label:"Línea inferior", render:(c:string) => (
                        <div className="px-2 py-2">
                          <p className="text-[9px] font-bold text-gray-700 tracking-widest uppercase mb-1">Educación</p>
                          <div className="h-0.5 rounded-full" style={{ background:c }}/>
                        </div>
                      )},
                      { key:"left-bar", label:"Barra lateral", render:(c:string) => (
                        <div className="px-2 py-2 flex items-center gap-1.5">
                          <div className="w-0.5 h-4 rounded-full" style={{ background:c }}/>
                          <p className="text-[9px] font-bold text-gray-700 tracking-widest uppercase">Educación</p>
                        </div>
                      )},
                      { key:"filled", label:"Relleno", render:(c:string) => (
                        <div className="mx-2 my-1.5 px-2 py-1 rounded-md" style={{ background:`${c}18` }}>
                          <p className="text-[9px] font-bold tracking-widest uppercase" style={{ color:c }}>Educación</p>
                        </div>
                      )},
                      { key:"minimal", label:"Minimalista", render:(_:string) => (
                        <div className="px-2 py-2">
                          <p className="text-[9px] font-bold text-gray-400 tracking-widest uppercase">Educación</p>
                        </div>
                      )},
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { key:"chips", label:"Etiquetas", render:(c:string) => (
                        <div className="flex flex-wrap gap-1 p-2">
                          {["React","Node","CSS"].map(s=>(
                            <span key={s} className="text-[8px] px-1.5 py-0.5 rounded-full font-medium"
                              style={{ background:`${c}18`, color:c, border:`1px solid ${c}33` }}>{s}</span>
                          ))}
                        </div>
                      )},
                      { key:"dots", label:"Puntos", render:(_:string) => (
                        <div className="p-2 space-y-0.5">
                          {["React","Node","CSS"].map(s=>(
                            <div key={s} className="flex items-center gap-1 text-[9px] text-gray-600">
                      ...
                      { key:"bars", label:"Barras", render:(c:string) => (
                        <div className="p-2 space-y-1.5">
                          {[["React",80],["Node",60]].map(([s,v])=>(
                            <div key={s as string}>
                              <p className="text-[8px] text-gray-500 mb-0.5">{s as string}</p>
                      ...
                      { key:"text", label:"Texto plano", render:(_:string) => (
                        <div className="p-2">
                          <p className="text-[9px] text-gray-500 leading-relaxed">React · Node · CSS · Git</p>
                        </div>
                      )},
                    ].map(({ key, label, render }) => (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button key={key} onClick={() => setCardStyle(key as any)}
                        className={cn(
                          "p-3 rounded-2xl border-2 text-left transition-all hover:shadow-sm",
                          cardStyle === key ? "border-green-500 bg-green-50" : "border-gray-100 hover:border-gray-200"
                        )}>
                        <div className="h-6 rounded-lg mb-1.5 flex items-center px-2"
                          style={{
                            background: key === "accent" ? `${accent}14` : bg,
                            border: `1px solid ${key === "bordered" ? border : "transparent"}`,
                            boxShadow: shadow,
                            borderLeft: key === "accent" ? `3px solid ${accent}` : undefined,
                          }}>
              ...
              {tab === "secciones" && (
                <>
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Visibilidad de secciones</p>
                    {hidden.size > 0 && (
                  ...
                  {sectionTypes.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-xs text-gray-400">Sin registros visibles aún</p>
                    ...
                    return (
                      <div key={type}
                        ...
                        className={cn(
                          "flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all select-none mb-2",
        ...
        <div className="flex flex-col min-h-0">
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full shadow-sm border border-white" style={{ background:accent }}/>
