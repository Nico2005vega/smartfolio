...
// ─────────────────────────────────────────────────────────────────
//  InstitutionSearch.tsx  —  v3 con API dinámica
//  · Ciudades: api-colombia.com  (todas las ciudades de Colombia)
//  · Colegios: datos.gov.co SIMAT (todos los colegios del MEN)
//  · Universidades/SENA: datos estáticos locales
// ─────────────────────────────────────────────────────────────────

import { useState, useEffect, useRef, useMemo, useCallback } from "react";
import {
  Search, MapPin, Building2, X, Check,
  ChevronDown, Loader2,
} from "lucide-react";
import {
  type Institution,
...
// ── Resultado unificado (estático + API) ─────────────────────────
...
export function InstitutionSearch({
  ...
  useEffect(() => {
    if (!selectedDept) { setApiCities([]); setCity(""); return; }

    setLoadingCities(true);
    setCity("");
    setApiCities([]);

    fetch(`/api/colombia/cities?dept=${encodeURIComponent(selectedDept)}`)
      .then((r) => r.json())
      .then((data) => setApiCities(data.cities ?? []))
      .catch(() => setApiCities([]))
      .finally(() => setLoadingCities(false));
  }, [selectedDept]);

  // ── Buscar colegios cuando hay ciudad o texto suficiente ─────
  const fetchSchools = useCallback(async (dept: string, city: string, q: string) => {
    // Solo busca colegios si hay ciudad seleccionada o texto ≥ 3 caracteres
    ...
    try {
      const r    = await fetch(`/api/colombia/schools?${params}`);
      const data = await r.json();
      setApiSchools(
        (data.schools ?? []).map((s: SearchResult, i: number) => ({
          ...s,
          id: `api-${i}-${s.name}`,
        }))
      );
  ...
  // ── Resultados combinados (estáticos + API) ──────────────────
  const results = useMemo<SearchResult[]>(() => {
    // Estáticos (universidades, SENA, institutos) — excluir colegios si la API los trae
    const staticItems = searchInstitutions(query, {
      department: selectedDept || undefined,
      city:       selectedCity || undefined,
      type:       selectedType || undefined,
    }, 20).filter((inst) =>
      apiSchools.length === 0 || inst.type !== "colegio"
    ) as SearchResult[];

    // Filtrar colegios de la API según tipo seleccionado
    const schoolItems: SearchResult[] =
      (!selectedType || selectedType === "colegio") ? apiSchools : [];

    // Combinar: universidades/SENA primero, luego colegios de API
    return [...staticItems, ...schoolItems].slice(0, 40);
  ...
  const isLoading    = loadingCities || loadingSchools;
      ...
      {isOpen && !disabled && (
        <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden">
          ...
          <div className="flex flex-wrap gap-2 p-3 bg-gray-50 border-b border-gray-100">
            ...
            {/* Ciudad — viene de la API */}
            <div className="relative flex-1 min-w-[130px]">
              {loadingCities ? (
                <div className="flex items-center gap-2 px-2.5 py-2 border border-gray-200 rounded-lg bg-white text-xs text-gray-400">
                  <Loader2 size={11} className="animate-spin" /> Cargando ciudades…
                </div>
              ) : (
                <>
                  <select
          ...
          <ul ref={listRef} className="max-h-64 overflow-y-auto divide-y divide-gray-50">
            {loadingSchools && results.length === 0 ? (
              <li className="px-4 py-6 text-center">
                ...
                <p className="text-sm text-gray-500">Buscando instituciones…</p>
              </li>
            ) : results.length === 0 ? (
              <li className="px-4 py-6 text-center">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className="flex items-center gap-0.5 text-xs text-gray-400">
                          <MapPin size={10} />
                          {inst.city}{inst.department ? `, ${inst.department}` : ""}
                        </span>
                        <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${typeColor}`}>
                          {typeLabel}
                        </span>
                        {inst.sector && (
                          <span className="text-xs text-gray-400 capitalize">{inst.sector}</span>
                        )}
                      </div>
          ...
          {selectedCity && !loadingSchools && (
            <div className="px-4 py-2 bg-green-50 border-t border-green-100">
              <p className="text-xs text-green-700">
                {apiSchools.length > 0
                  ? `${apiSchools.length} colegio${apiSchools.length !== 1 ? "s" : ""} encontrado${apiSchools.length !== 1 ? "s" : ""} en ${selectedCity}`
                  : `No se encontraron colegios en ${selectedCity} — prueba buscando por nombre`
                }
              </p>
            </div>
          )}
