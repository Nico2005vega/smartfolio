...
export default function CVPreviewModern({ data }: Props) {
  ...
  /* ── Customization values (with sensible defaults) ── */
  ...
  const basePx       = cfg.font_size    ?? 13;
  const lh           = cfg.line_height  ?? 1.55;
  const photoShape   = cfg.photo_shape  ?? "circle";
  const sectionStyle = cfg.section_style ?? "underline";
  const skillsStyle  = cfg.skills_style  ?? "chips";
  const cardStyle    = cfg.card_style    ?? "flat";
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"16px" }}>
            {sections.map(section=>(
              <div key={section.type}>
                ...
                {section.records.map(r=><RecordCard key={r.id} r={r}/>)}
              </div>
            ))}
          </div>
        </div>
