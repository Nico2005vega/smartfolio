...
function SecHead({ icon, label, cfg }: { icon:string; label:string; cfg:Cfg }) {
  const { showIcons, secStyle, px, accent, font, mono } = cfg;
...
function Card({ r, cfg }: { r:AcademicRecord; cfg:Cfg }) {
  const { font, mono, cardSt, accent, px, lh } = cfg;
...
function SidebarSkills({ skills, cfg }: { skills:Record<string,Skill[]>; cfg:Cfg }) {
  const { skillsSt, px, accent, mono } = cfg;
...
export default function CVPreviewTech({ data }: Props) {
  ...
  const { accent, font, mono, px, lh, photoR, showPhoto } = cfg;
